...
public class HttpExternalNotifier implements ExternalNotifier {
    ...
    public void sendPaymentNotify(UserProfile userProfile, BigInteger amount, PaymentStatus status, String txHash, CryptoCurrency currency, boolean isSuccess) {
        doPost(paymentUrl, new PaymentNotify(userProfile.getUser().getId(), amount, status, txHash, currency, isSuccess));
    }
    ...
    public void sendDeployedNotification(Contract contract, String address, String transactionHash, boolean committed, boolean status) {
        doPost(deployed, new ContractDeployedNotify(contract.getId(), committed ? PaymentStatus.COMMITTED : PaymentStatus.REJECTED, address, transactionHash, status));
    }
